...
def main():
    ...
    with st.sidebar:
        ...
        with st.expander("Add Source", expanded=True):
            source_type = st.selectbox("Type", ["traffic", "industry", "household"])
            ...
            emission_rate = st.number_input("Emission Rate (kg/hour)", 0.0, 100.0, 10.0)
            intensity = st.number_input("Intensity", 0.5, 2.0, 1.0)
            active_hours = st.text_input("Active Hours", "08:00-18:00")
        ...
        with st.expander("Add Intervention"):
            intervention_type = st.selectbox("Type", ["vertical_garden", "roadside_unit", "biofilter"])
            int_x = st.slider("X", 0, 50, 25, key="int_x")
            int_y = st.slider("Y", 0, 50, 25, key="int_y")
            int_z = st.slider("Z", 0, 20, 0, key="int_z")
            efficiency = st.number_input("Efficiency", 0.0, 1.0, 0.5)
            radius = st.number_input("Radius", 1, 20, 5)
            cost = st.number_input("Cost (USD)", 1000, 100000, 5000)
            capacity = st.number_input("Capacity (kg/hour)", 10, 100, 50)
        ...
        with st.expander("Add Building"):
            x_min = st.number_input("X_min", 0, 50, 10)
            x_max = st.number_input("X_max", 0, 50, 20)
            y_min = st.number_input("Y_min", 0, 50, 10)
            y_max = st.number_input("Y_max", 0, 50, 20)
            z_min = st.number_input("Z_min", 0, 20, 0)
            z_max = st.number_input("Z_max", 0, 20, 10)
            material = st.selectbox("Material", ["concrete", "glass"])
        ...
        with st.expander("Scenario Save/Load"):
            ...
            scenario_load = st.selectbox("Load Scenario", ["None"] + get_data("scenarios").keys())
            if scenario_load != "None":
                load_scenario(scenario_load)

        # Simulation Settings
        with st.expander("Simulation Settings"):
            use_api = st.checkbox("Use OpenWeatherAPI for wind")
            wind_speed = st.number_input("Wind Speed (m/s)", 0.0, 50.0, 5.0) if not use_api else None
            wind_direction = st.number_input("Wind Direction (degrees)", 0, 360, 0) if not use_api else None
            time_steps = st.number_input("Time Steps", 1, 24, 24)
    ...
    with tab1:
        ...
        compare_run = st.selectbox("Compare Simulation", ["None"] + list(st.session_state.simulations.keys()))
        col1, col2 = st.columns(2) if compare_run != "None" else (st, None)
        with col1:
            st.plotly_chart(plot_3d_co2(st.session_state.simulation_results, time_step))
            st.plotly_chart(plot_2d_slice(st.session_state.simulation_results, st.slider("Z Slice", 0, 20, 0), time_step))
            st.plotly_chart(plot_time_series(st.session_state.simulation_results))
            st.write(create_folium_map(st.session_state.simulation_results, time_step))
            map_style = st.selectbox("Map Style", ["OpenStreetMap", "Stamen Terrain", "CartoDB Positron"])
            layer_toggles = st.multiselect("Map Layers", ["CO2 Heatmap", "Sources", "Interventions", "Buildings", "Sensors"])
            st.slider("Map Zoom", 10, 18, 13)
